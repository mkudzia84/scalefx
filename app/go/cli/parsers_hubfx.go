...
func ParseAudioStatus(payload []byte) {
	...
	sampleRate := protocol.ReadU16LE(payload, pos)
	pos += 2
	bitDepth := payload[pos]
	pos++
	maxChannels := payload[pos]
	pos++
	codecNameLen := int(payload[pos])
	pos++
	codecName := ""
	...
	if hasRingStats && pos+9 <= len(payload) {
		ringFillPct = payload[pos]
		pos++
		ringAvailRead = protocol.ReadU16LE(payload, pos)
		pos += 2
		ringAvailWrite = protocol.ReadU16LE(payload, pos)
		pos += 2
		underruns = protocol.ReadU32LE(payload, pos)
		pos += 4
		if pos+8 <= len(payload) {
			consumeLoops = protocol.ReadU32LE(payload, pos)
			pos += 4
			consumeFrames = protocol.ReadU32LE(payload, pos)
			pos += 4
		}
	}
	...
	if hasBufferCaps && pos+4 <= len(payload) {
		wavBufCapacity = protocol.ReadU16LE(payload, pos)
		pos += 2
		ringCapacity = protocol.ReadU16LE(payload, pos)
		pos += 2
	}
	...
	for i := 0; i < activeCount; i++ {
		...
		vol := payload[pos]
		pos++
		playing := payload[pos] != 0
		pos++
		looping := payload[pos] != 0
		pos++
		loopCount := protocol.ReadU16LE(payload, pos)
		pos += 2
		remaining_ms := protocol.ReadU32LE(payload, pos)
		pos += 4
		queueLen := payload[pos]
		pos++
		output := payload[pos]
		pos++

		wavRate := protocol.ReadU16LE(payload, pos)
		pos += 2
		wavCh := payload[pos]
		pos++
		wavBits := payload[pos]
		...
		if hasBufferCaps && pos < len(payload) {
			wavBufFill = payload[pos]
			pos++
		}

		fnameLen := int(payload[pos])
		pos++
		fname := ""
...
func ParseUsbDevices(payload []byte) {
	...
	pos := 0
	initialized := payload[pos] != 0
	pos++
	taskRunning := payload[pos] != 0
	pos++
	backendLen := int(payload[pos])
	pos++
	...
	backend := string(payload[pos : pos+backendLen])
	pos += backendLen
	if pos >= len(payload) {
		fmt.Println("  (malformed USB response)")
		return
	}
	deviceCount := int(payload[pos])
	pos++
	...
	for i := 0; i < deviceCount; i++ {
		...
		addr := payload[pos]
		pos++
		vid := protocol.ReadU16LE(payload, pos)
		pos += 2
		pid := protocol.ReadU16LE(payload, pos)
		pos += 2
		state := payload[pos]
		pos++
		slaveType := payload[pos]
		pos++
...
func ParseSlaveInfo(payload []byte) {
	...
	if pos+4 <= len(payload) {
		cpuMHz = protocol.ReadU32LE(payload, pos)
		pos += 4
	}
	if pos+4 <= len(payload) {
		freeRAM = protocol.ReadU32LE(payload, pos)
		pos += 4
	}
	if pos+4 <= len(payload) {
