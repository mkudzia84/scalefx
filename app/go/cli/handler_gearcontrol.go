...
func (c *CLI) cmdGcDeploy(args []string) {
	...
	gc := c.api.GearControl
	if strings.ToLower(args[0]) == "all" {
		c.ack(gc.AllDeploy(), "Deploy ALL gears")
	} else {
		id := atoi(args[0])
		c.ack(gc.Deploy(byte(id)), fmt.Sprintf("Deploy gear %d (%s)", id, gearIDName(byte(id))))
	}
}
...
func (c *CLI) cmdGcRetract(args []string) {
	...
	gc := c.api.GearControl
	if strings.ToLower(args[0]) == "all" {
		c.ack(gc.AllRetract(), "Retract ALL gears")
	} else {
		id := atoi(args[0])
		c.ack(gc.Retract(byte(id)), fmt.Sprintf("Retract gear %d (%s)", id, gearIDName(byte(id))))
	}
}
...
func (c *CLI) cmdGcStop(args []string) {
	...
	gc := c.api.GearControl
	if strings.ToLower(args[0]) == "all" {
		c.ack(gc.AllStop(), "STOP ALL motors")
	} else {
		id := atoi(args[0])
		c.ack(gc.Stop(byte(id)), fmt.Sprintf("STOP motor %d", id))
	}
}

// Servo commands delegate to shared helpers (Strategy pattern)
func (c *CLI) cmdGcServo(args []string) {
	c.servoSet(args, "servo set <id> <pulse_us>", c.api.GearControl.ServoSet)
}

func (c *CLI) cmdGcServoConfig(args []string) {
	c.servoConfig(args, "servo.config <id> <min> <max> [spd] [acc] [dec]", c.api.GearControl.ServoConfig)
}
...
func (c *CLI) cmdGcGearConfig(args []string) {
	...
	c.ack(c.api.GearControl.GearConfig(byte(id), byte(flags), uint16(stall), uint16(timeout)),
...
func (c *CLI) cmdGcDoorConfig(args []string) {
	...
	c.ack(c.api.GearControl.DoorConfig(byte(id), uint16(o0), uint16(c0), uint16(o1), uint16(c1)),
...
func (c *CLI) cmdGcDoorMode(args []string) {
	...
	c.ack(c.api.GearControl.DoorMode(byte(id), mode, postDeploy, delay_ms),
...
func (c *CLI) cmdGcYawConfig(args []string) {
	...
	c.ack(c.api.GearControl.YawConfig(byte(id), uint16(neutral), uint16(min), uint16(max)),
...
func (c *CLI) cmdGcYaw(args []string) {
	...
	pos := atoi(args[0])
	c.ack(c.api.GearControl.YawInput(uint16(pos)), fmt.Sprintf("Yaw → %dµs", pos))
}
...
func (c *CLI) cmdGcReset(args []string) {
	...
	gc := c.api.GearControl
	c.forEachGear(args[0], func(id byte) {
		c.ack(gc.Reset(id), fmt.Sprintf("Reset gear %d (%s)", id, gearIDName(id)))
	})
}
...
func (c *CLI) cmdGcEnable(args []string) {
	...
	gc := c.api.GearControl
	c.forEachGear(args[0], func(id byte) {
		c.ack(gc.Enable(id, true), fmt.Sprintf("Enabled gear %d (%s)", id, gearIDName(id)))
	})
}
...
func (c *CLI) cmdGcDisable(args []string) {
	...
	gc := c.api.GearControl
	c.forEachGear(args[0], func(id byte) {
		c.ack(gc.Enable(id, false), fmt.Sprintf("Disabled gear %d (%s)", id, gearIDName(id)))
	})
}
...
func (c *CLI) cmdGcBattery(args []string) {
	...
	c.ack(c.api.GearControl.BatteryConfig(enabled, autoDeploy), fmt.Sprintf("Battery monitoring: %s", state))
}
