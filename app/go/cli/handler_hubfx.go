...
func (c *CLI) cmdHubSlaves(_ []string) {
	c.query(c.api.HubFx.SlaveList(), ParseSlaveList)
}
...
func (c *CLI) cmdHubSlaveInit(args []string) {
	...
	c.ack(c.api.HubFx.SlaveInit(slaveType), fmt.Sprintf("%s slave initialized", hubfx.SlaveTypeName(slaveType)))
}
...
func (c *CLI) cmdHubSlaveInfo(args []string) {
	...
	c.query(c.api.HubFx.SlaveInfo(slaveType), ParseSlaveInfo)
}
...
func (c *CLI) cmdHubAudioPlay(args []string) {
	...
	c.ack(c.api.HubFx.AudioPlay(ch, vol, output, loopMode, loopCount, path),
...
func (c *CLI) cmdHubAudioStop(args []string) {
	...
	c.ack(c.api.HubFx.AudioStop(ch), fmt.Sprintf("Audio stop %s", target))
}
...
func (c *CLI) cmdHubAudioVol(args []string) {
	...
	c.ack(c.api.HubFx.AudioVolume(ch, vol), fmt.Sprintf("Volume %s → %d%%", target, vol))
}
...
func (c *CLI) cmdHubAudioFade(args []string) {
	...
	c.ack(c.api.HubFx.AudioFade(byte(atoi(args[0]))), fmt.Sprintf("Fade out ch%s", args[0]))
}
...
func (c *CLI) cmdHubAudioQueue(args []string) {
	...
	c.ack(c.api.HubFx.AudioQueue(ch, vol, loopCount, loopBehavior, path),
...
func (c *CLI) cmdHubAudioClear(args []string) {
	...
	c.ack(c.api.HubFx.AudioQueueClear(ch), fmt.Sprintf("Queue cleared %s", target))
}

func (c *CLI) cmdHubAudioStatus(_ []string) {
	c.query(c.api.HubFx.AudioStatus(), ParseAudioStatus)
}

func (c *CLI) cmdHubCodecStatus(_ []string) {
	c.query(c.api.HubFx.CodecStatus(), ParseCodecStatus)
}

// ─── Engine Commands ───

func (c *CLI) cmdHubEngineStart(_ []string) {
	c.ack(c.api.HubFx.EngineStart(), "Engine start")
}
func (c *CLI) cmdHubEngineStop(_ []string) {
	c.ack(c.api.HubFx.EngineStop(), "Engine stop")
}

func (c *CLI) cmdHubEngineStatus(_ []string) {
	c.query(c.api.HubFx.EngineStatus(), ParseEngineStatus)
}
...
func (c *CLI) cmdHubConfigReload(args []string) {
	...
	c.ack(c.api.HubFx.ConfigReload(path), "Config reload")
}

func (c *CLI) cmdHubConfigStatus(_ []string) {
	c.query(c.api.HubFx.ConfigStatus(), ParseConfigStatus)
}
...
func (c *CLI) cmdHubConfigSave(args []string) {
	...
	c.ack(c.api.HubFx.ConfigSave(path), "Config save")
}

// ─── Storage Commands ───

func (c *CLI) cmdHubSdInit(_ []string) { c.ack(c.api.HubFx.SdInit(), "SD card remounted") }

func (c *CLI) cmdHubSdStatus(_ []string) {
	c.query(c.api.HubFx.SdStatus(), ParseSdStatus)
}

func (c *CLI) cmdHubFlashStatus(_ []string) {
	c.query(c.api.HubFx.FlashStatus(), ParseFlashStatus)
}

func (c *CLI) cmdHubUsbDevices(_ []string) {
	c.query(c.api.HubFx.UsbDevices(), ParseUsbDevices)
}

func (c *CLI) cmdHubUsbReset(_ []string) { c.ack(c.api.HubFx.UsbReset(), "USB bus reset") }
...
func (c *CLI) cmdHubFileList(args []string) {
	...
	text, err := c.api.Files.List(target, path)
	if err != nil {
		PrintError("%v", err)
		return
	}
...
func (c *CLI) cmdHubFileDelete(args []string) {
	...
	c.ack(c.api.Files.Delete(target, path), fmt.Sprintf("Delete %s:%s", storageTargetName(target), path))
}
...
func (c *CLI) cmdHubFileMkdir(args []string) {
	...
	c.ack(c.api.Files.Mkdir(target, path), fmt.Sprintf("Mkdir %s:%s", storageTargetName(target), path))
}
...
func (c *CLI) cmdHubFileInfo(args []string) {
	...
	r := c.api.Files.Info(target, path)
	if !r.OK {
...
func (c *CLI) cmdHubFileTree(args []string) {
	...
	text, err := c.api.Files.Tree(target, path)
	if err != nil {
		PrintError("%v", err)
		return
	}
...
func (c *CLI) cmdHubFileCat(args []string) {
	...
	text, err := c.api.Files.Cat(target, path)
	if err != nil {
		PrintError("%v", err)
		return
	}
...
func (c *CLI) cmdHubFileDownload(args []string) {
	...
	result, err := c.api.Files.Download(target, remotePath, 60*time.Second)
	if err != nil {
		PrintError("%v", err)
		return
	}
...
func (c *CLI) cmdHubFileUpload(args []string) {
	...
	result := c.api.Files.Upload(target, remotePath, fileData, mode,
		func(sent, total int) {
...
func (c *CLI) cmdHubFileCancel(_ []string) {
	c.ack(c.api.Files.CancelUpload(), "Upload cancelled")
}
