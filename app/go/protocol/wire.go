...
func BuildPacket(packetType PacketType, payload []byte, tag byte) []byte {
	length := len(payload)
	raw := make([]byte, 0, 4+length+1)
	raw = append(raw, byte(packetType), tag, byte(length&0xFF), byte((length>>8)&0xFF))
...
func ParsePacket(data []byte) (PacketType, byte, []byte, bool) {
