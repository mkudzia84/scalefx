...
shared `ServoWidget` embedded inline in the **feature panel** (GunFx
Turret section, future EngineFx servo binding) so operators tune the
servo where they wire the feature.  (An IO-tab calibrate/reset affordance
was tried 2026-06-01 and removed 2026-06-02 — the IO tab stays a pure
port/role/name surface.)
...
> to Svelte.
>
> ⚠️ **GUID gotcha (cost me several rounds):** `Topology.PortList("")` is a
> *request* sentinel (`"" → the hub`), but the RESPONSE denormalizes each
> board's REAL GUID, so `BuildModel` gives hub-local ports the hub's GUID
> (e.g. `6D60`), NOT `""`.  Gating any servo/port UI on `p.ref.guid === ""`
> silently matches nothing on the hub.  The hub self-routes a topology
> forward addressed to its own GUID, so live jog/push work for `6D60`.
