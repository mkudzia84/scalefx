...
inline void Gear::commandStrutServo() {
    if (!_send) return;
    // ABSOLUTE pulse targets (2.46.0 — explicit beats implicit): deploy and
    // retract are configured µs numbers, clamped by the servo's calibration
    // at the role.  The 0xFFFF/0 defaults land on the calibrated ends, and
    // there is no direction flag ANYWHERE in this path any more (the
    // three-layer reversed stack died in the 2026-08-08 bench saga).
    const uint16_t targetUs = (_target == Target::Down) ? _def.deployUs
                                                        : _def.retractUs;
    uint8_t payload[3];
