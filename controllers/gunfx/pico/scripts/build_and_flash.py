#!/usr/bin/env python3
"""
GunFX Pico - Build and Flash Script

Performs verified build, automatic BOOTSEL entry via text protocol, and firmware upload
...
Note:
    This script is for flashing GunFX when connected directly to PC via USB.
    It uses the text protocol (INIT protocol=text) to establish communication
    and send the BOOTSEL command.
"""
...
def extract_version_from_source(project_dir: Path) -> Tuple[str, int]:
    ...
    source_file = project_dir / "src" / "gunfx_pico.ino"
    ...
    if source_file.exists():
        ...
        # Extract version: #define FIRMWARE_VERSION "0.2.0"
        version_match = re.search(r'#define\s+FIRMWARE_VERSION\s+"([^"]+)"', content)
        if version_match:
            version = version_match.group(1)
        
        # Extract build number: #define BUILD_NUMBER 1
...
def verify_firmware(project_dir: Path) -> Optional[BuildInfo]:
    ...
    # Get version and build number from source
...
def send_init_and_bootsel(port: str) -> bool:
    """
    Send INIT with text protocol, then BOOTSEL command to trigger BOOTSEL mode.
    
    Uses the text protocol:
    1. INIT protocol=text keepalive=off  -> establishes text mode
    2. Wait for INIT_READY response
    3. BOOTSEL -> triggers bootloader mode
    """
    if not SERIAL_AVAILABLE:
    ...
    try:
        ...
        # Step 1: Send INIT command with text protocol
        print_info("Sending INIT protocol=text...")
        ser.write(b"INIT protocol=text keepalive=off\r\n")
        ser.flush()
        
        # Wait for INIT_READY response
        time.sleep(0.5)
        response = ser.read(ser.in_waiting or 256).decode('utf-8', errors='ignore')
        
        if "INIT_READY" in response:
            print_info("Received INIT_READY - device initialized in text mode")
            
            # Parse device info from INIT_READY
            # Format: INIT_READY name=GunFX-XXXX version=0.2.0 platform=RP2040 ...
            name_match = re.search(r'name=(\S+)', response)
            version_match = re.search(r'version=(\S+)', response)
            if name_match and version_match:
                print_info(f"Device: {name_match.group(1)} v{version_match.group(1)}")
        else:
            print_info("No INIT_READY response (device may already be initialized)")
        
        # Step 2: Send BOOTSEL command
        print_info("Sending BOOTSEL command...")
        ser.write(b"BOOTSEL\r\n")
        ser.flush()
...
def verify_device_version(port: str, expected_version: str) -> Tuple[bool, str, int]:
    """
    Connect to device using text protocol and verify firmware version.
    Returns (success, actual_version, actual_build)
    Only validates version match, build number is just for display.
    """
    if not SERIAL_AVAILABLE:
        return False, "", 0
    ...
    try:
        ...
        # Format: INIT_READY name=GunFX-XXXX version=0.2.0 build=1 platform=RP2040 cpuMHz=120 ramBytes=...
        version_match = re.search(r'version=(\S+)', response)
        build_match = re.search(r'build=(\d+)', response)
...
def main():
    parser = argparse.ArgumentParser(description="GunFX Pico Build and Flash Utility")
    ...
    print_header("GunFX Pico - Build and Flash Utility")
    print_info("Connect GunFX directly to PC via USB for flashing")
    ...
    # First check if already in BOOTSEL mode (vanilla/unflashed or manual BOOTSEL)
    # This prioritizes controllers that are already waiting to be flashed
    drive = find_bootsel_drive()
    
    if drive:
        print_ok("Device already in BOOTSEL mode (vanilla or manual)")
        print_info(f"Drive: {drive}")
    else:
        # No BOOTSEL drive found - try to connect via serial and send BOOTSEL command
        serial_port = args.port or find_pico_serial_port()
        
        if serial_port:
            print_info(f"Found device at {serial_port}")
            print_info("Device has firmware - sending BOOTSEL command...")
            send_init_and_bootsel(serial_port)
            
            # Wait for BOOTSEL drive
            drive = wait_for_bootsel_drive(args.timeout)
        else:
            print_info("No serial port found - waiting for BOOTSEL drive...")
            print_info("(Connect a vanilla Pico or hold BOOTSEL + RESET)")
            drive = wait_for_bootsel_drive(args.timeout)
    ...
    if not args.skip_verify:
        ...
        # Wait for serial port to reappear (or appear for first time if vanilla)
        port = wait_for_serial_port(None, timeout=15)  # Always auto-detect after flash
        ...
        # Verify version using text protocol (only version is validated, build is for display)
        success, actual_version, actual_build = verify_device_version(port, build_info.version)
        
        if success:
            print_ok(f"Version verified: {actual_version} (Build {actual_build})")
        elif actual_version:
            print_err(f"Version mismatch!")
            print_info(f"Expected: {build_info.version}")
