...
#elif defined(ARDUINO_ARCH_ESP32)
    static i2s_chan_handle_t _i2sTxHandle = nullptr;
...
bool AudioMixer::beginI2S() {
...
#elif defined(ARDUINO_ARCH_ESP32)
    // ESP32-S3: ESP-IDF I2S standard mode driver
    i2s_chan_config_t chan_cfg = {
        .id = I2S_NUM_0,
        .role = I2S_ROLE_MASTER,
        .dma_desc_num = 8,
        .dma_frame_num = 512,     // 8×512×4 = 16 KB DMA buffer
        .auto_clear = true,
    };
    
    esp_err_t err = i2s_new_channel(&chan_cfg, &_i2sTxHandle, nullptr);
    if (err != ESP_OK) {
    ...
    i2s_std_config_t std_cfg = {
        ...
        .gpio_cfg = {
            ...
            .bclk = (gpio_num_t)_i2sBclkPin,
            .ws   = (gpio_num_t)_i2sLrclkPin,
            .dout = (gpio_num_t)_i2sDataPin,
            .din  = I2S_GPIO_UNUSED,
            .invert_flags = {
                .mclk_inv = false,
                .bclk_inv = false,
    ...
    err = i2s_channel_init_std_mode(_i2sTxHandle, &std_cfg);
    if (err != ESP_OK) {
...
bool AudioMixer::play(int channel, const char* filename, const AudioPlaybackOptions& options) {
    ...
    if (!ws.file.open(&SD_CARD().getSd(), filename, O_RDONLY)) {
...
void AudioMixer::consumeAndOutput() {
    ...
    if (avail > 0) {
        ...
        i2s_channel_write(_i2sTxHandle, _i2sBatchBuf, count * 4, &bytesWritten, portMAX_DELAY);
        _consumeFrames.fetch_add(count, std::memory_order_relaxed);
#endif
    } else {
        ...
        size_t bytesWritten = 0;
        i2s_channel_write(_i2sTxHandle, silence, 4, &bytesWritten, portMAX_DELAY);
#endif
    }
}
