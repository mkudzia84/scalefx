...
│  │  client/usb_host.h (PIO-USB)     │                           │
│  └──────────────────────────────────┘                           │
│                                                                  │
│  ┌──────────────────────────────────┐                           │
│  │     client/result_queue.h        │                           │
│  │  ResultQueue (tag correlation)   │  ← Client infrastructure  │
│  └──────────────────────────────────┘                           │
│                                                                  │
│  ┌───────────────────┐ ┌──────────────────┐ ┌──────────────────┐│
│  │ gunfx/gunfx.h    │ │ lightfx/lightfx.h │ │ serial_gear-     ││
...
```
ICommandHandler (interface in core/core.h)
  │ tryProcess(type, payload, len) → CommandHandleResult
  │ handlerName() → const char*
  │
  └── BusServer (core/bus_server.h — ACK/NACK helpers, range routing)
        │ begin(Stream*) / end()
...
StreamWriter (core/stream.h — uses BusServer for packet output)
...
```
SerialBus (client/bus.h — COBS framing over USB CDC)
  │ begin(UsbHost*, port) / process()
  │ sendPacket(type, payload, len, tag)
  │
  └── BusClient (client/bus_client.h — INIT handshake, ACK/NACK, tag queue)
...
| `core/core.h` | Protocol, errors, ICommandHandler, CommandRouter, SFX macros | All |
| `core/core.cpp` | CoreProtocol implementations, CorePayload encode/decode | All |
| `core/bus_server.h` | BusServer base + CoreCommandServer | Server only |
| `core/bus_server.cpp` | BusServer + CoreCommandServer implementations | Server only |
| `client/bus_client.h` | BusClient base class (extends SerialBus) | Client only |
| `client/bus_client.cpp` | BusClient implementation | Client only |
| `client/bus.h` | SerialBus (USB Host COBS transport) | Client only |
| `client/bus.cpp` | SerialBus implementation | Client only |
| `client/usb_host.h` | UsbHost (PIO-USB manager) | Client only |
| `client/result_queue.h` | ResultQueue (tag-correlated request/response) | Client only |
| `client/result_queue.cpp` | ResultQueue implementation | Client only |
| `core/stream.h` | StreamProtocol constants + StreamWriter (chunked streaming) | Server only |
| `core/stream.cpp` | StreamWriter + CRC-16/CCITT implementation | Server only |
| `core/diag_log.h` | DiagLog — diagnostic log output over serial protocol | All |
| `core/diag_log.cpp` | DiagLog implementation (ring buffer, COBS flush, ingest) | All |
| `gunfx/gunfx.h/.cpp` | GunFxServer + GunFxClient + GunFxPacket + GunFxError + GunFxSpec | GunFX |
| `lightfx/lightfx.h/.cpp` | LightFxServer + LightFxClient + LightFxPacket + LightFxError | LightFX |
| `gearcontrol/gearcontrol.h/.cpp` | GearControlServer + GearControlClient + GearControlPacket + GearControlError | GearControl |
...
- `0xA4-0xA6` — Streaming protocol (STREAM_BEGIN/DATA/END) — see `core/stream.h`
...
This guards `client/bus.h/.cpp` and USB Host includes so they are only compiled for the HubFX client.

## Error Handling

Error codes are organized by namespace:

- **`SerialError`** (core/core.h) — Generic errors (0x00-0x1F) and system errors (0xF0-0xFF)
- **`GunFxError`** (gunfx/gunfx.h) — GunFX-specific errors (0x20-0x4F)
- **`LightFxError`** (lightfx/lightfx.h) — LightFX-specific errors (0x50-0x5F)
- **`GearControlError`** (gearcontrol/gearcontrol.h) — GearControl-specific errors (0x60-0x6F)
