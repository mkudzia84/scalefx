...
private:
    ...
    static void SFX_IRAM_FUNC isrHandler(void* arg);
    ...
    // Sample buffer for averaging (atomic for ISR safety + multi-core portability)
    std::atomic<int> _samples[PwmInputConfig::SAMPLE_COUNT]{};
    std::atomic<int> _sampleIndex{0};
    std::atomic<int> _sampleCount{0};
    
    // Latest reading
    std::atomic<int> _latestUs{0};              // ISR writes, main loop reads
    
    // Hysteresis state
    std::atomic<bool> _thresholdState{false};    // ISR writes, main loop reads
    int _lastThreshold = 0;
    int _configuredThreshold = 0;
    int _configuredHysteresis = PwmInputConfig::DEFAULT_HYSTERESIS_US;
    
    // Timing
    std::atomic<unsigned long> _lastUpdateUs{0}; // ISR writes, main loop reads
    std::atomic<unsigned long> _pulseStartUs{0}; // ISR-only
