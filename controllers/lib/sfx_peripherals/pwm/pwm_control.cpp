...
void PwmInput::begin(PwmInputType type, int pin) {
    ...
    // Clear sample buffer
    for (int i = 0; i < PwmInputConfig::SAMPLE_COUNT; i++) {
        _samples[i].store(0, std::memory_order_relaxed);
    }
    _sampleIndex.store(0, std::memory_order_relaxed);
    _sampleCount.store(0, std::memory_order_relaxed);
    _latestUs.store(0, std::memory_order_relaxed);
    _lastUpdateUs.store(0, std::memory_order_relaxed);
...
bool PwmInput::beginAsync(PwmInputType type, int pin) {
    ...
    SFX_ATTACH_INTERRUPT_PARAM(pin, isrHandler, CHANGE, this);
...
void PwmInput::reset() {
    ...
    _sampleCount.store(0, std::memory_order_relaxed);
    _latestUs.store(0, std::memory_order_relaxed);
    _thresholdState.store(false, std::memory_order_relaxed);
    _lastCallbackValue = 0;
    interrupts();
}
...
void SFX_IRAM_FUNC PwmInput::isrHandler(void* arg) {
