 ...
 * that the LED protocol server can work with any channel count or driver
 * backend without templates.
 *
 * Template parameters:
 *   MaxChannels  — Number of LED channels (compile-time constant)
 *   TDriver      — Hardware LED driver policy (default: GpioLedDriver)
 *
 ...
 *
 * Usage (LightFX Pico — 8 GPIO channels):
 *   LedManager<8, GpioLedDriver> ledManager;
 *   const uint8_t pins[8] = {0,1,2,3,4,5,6,7};
 *   ledManager.begin(pins);
 *   ledManager.ledSet(1, 80);            // ch 1 → 80%
 *   ledManager.seqStart(0);              // start all
 *   ledManager.update();                 // call in loop()
 *
 * Usage (HubFX ESP32-S3 — 6 expander channels):
 *   LedManager<6, ExpanderLedDriver> ledManager;
 *   // init channels via ledManager.channel(i).driver().begin(...)
 *   ledManager.beginConfigured();
 ...
 *
 * Used by LedProtocolServer so it can work with any LedManager<N, TDriver>
...
// ============================================================================
// LedManager<MaxChannels, TDriver> — Concrete LED channel manager
// ============================================================================

/**
 * @brief Template LED channel manager with pluggable driver backend.
 *
 ...
 */
template <uint8_t MaxChannels, typename TDriver = GpioLedDriver>
...
public:
    ...
    /**
     * @brief Initialize channels on GPIO pins (GpioLedDriver convenience).
     *
     * Calls channel(i).begin(pins[i], activeLow, pwm) for each channel
     ...
     */
    void begin(const uint8_t* pins, bool activeLow = false, bool pwm = true) {
        for (uint8_t i = 0; i < MaxChannels; i++) {
            _channels[i].begin(pins[i], activeLow, pwm);
            _channels[i].off();
    ...
    /**
     * @brief Finalize setup after drivers have been configured externally.
     *
     * For non-GPIO drivers (expander, etc.), the caller configures each
     * channel's driver before calling this method.  This method attaches
     * event sequences to channels and marks all enabled.
     */
    ...
    LedControlT<TDriver>&       channel(uint8_t index)       { return _channels[index]; }
    const LedControlT<TDriver>& channel(uint8_t index) const { return _channels[index]; }
...
private:
    LedControlT<TDriver> _channels[MaxChannels];
