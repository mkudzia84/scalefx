...
void IndicatorServicePolicy::update() {
    ...
    if (_watchdogTriggered) {
        ...
        _leds[0].set((SFX_MILLIS() / BLINK_WAITING_ms) % 2);
    } else {
    ...
    if (_errorCondition) {
        _leds[1].set((SFX_MILLIS() / BLINK_ERROR_ms) % 2);     // Fast blink: error
    } else if (_warningCondition) {
        _leds[1].set((SFX_MILLIS() / BLINK_WARNING_ms) % 2);   // Slow blink: warning
    } else {
