...
public:
     ...
     */
    bool isWaiting() const { return _waitingTag.load(std::memory_order_relaxed) != 0; }

    /**
     * @brief Get the tag currently being waited on (0 if none)
     */
    uint8_t waitingTag() const { return _waitingTag.load(std::memory_order_relaxed); }
...
private:
    ...
    // Blocking wait state
    std::atomic<uint8_t> _waitingTag{0};   // Same-core busy-wait (atomic for correctness, relaxed ordering)
    std::atomic<bool> _waitResolved{false}; // Same-core busy-wait (atomic for correctness, relaxed ordering)
