...
- **Aligned-row control height (design rule):** every control AND every button that shares a control/action row is `--control-h` tall, so the row reads as a single flush line. `.field-input` is pinned to `--control-h` globally; any `<button>` inside a `.form-row` / `.form-grid` / **`.header-actions`** / **`.status-row`** is auto-pinned to `--control-h` + vertically centred by the shared stylesheet — so `× Remove` / `+ Add` / `⚙ Calibrate` / `Deploy` never sit taller (base button) or shorter (`.small`) than the controls/pills beside them, AND a panel's header buttons match its form-row buttons. Buttons in free-form content (dialogs) keep their natural size. Retune every aligned row by changing the one `--control-h` token; never hand-set a button height in a component to match a control.
...
**Shared component (MANDATORY 2026-05-23):** the markup + styling are factored into [`ChannelToggleCluster.svelte`](../app/go/studio/frontend/src/lib/components/ChannelToggleCluster.svelte). **Every new channel-gated form imports it instead of inlining the rows** — props: `channelLabel`, `emptyOption`, `options=[{id,label}]`, `inputId`, `thresholdUs`, `hysteresisUs`, `liveUs`, `liveValid`, `actionVerb` ("Fires" / "Triggers" / "Switches"), `onChange({inputId, thresholdUs, hysteresisUs})`. Reference call sites: [EnginePanel.svelte](../app/go/studio/frontend/src/lib/tabs/EnginePanel.svelte) (engine on/off), [GunFxPanel.svelte](../app/go/studio/frontend/src/lib/tabs/GunFxPanel.svelte) per-gun Trigger section, [LandingPanel.svelte](../app/go/studio/frontend/src/lib/tabs/LandingPanel.svelte) (a landing group's RC-channel auto-deploy — `actionVerb="Deploys"`). **Any group/effect whose activation is "bound to an RC channel" MUST render this widget so the operator sees the live channel value + threshold, never a bare select + number input.** Inlining the 60-line block again is a Rule 36 violation. **Full walkthrough:** [21-STUDIO-ENGINEFX-PANEL.md § 2](../instructions/21-STUDIO-ENGINEFX-PANEL.md) (the bar with overlays — design rationale, z-order, colour semantics, reuse checklist).
...
**Normalised-position intent verb (`SERVO_SET_POS_NORM` 0x55, added 2026-06-06).** When an effect wants "drive this servo to a fraction of its travel" it sends a position fraction `[0, kPosNormFull=10000]` (0 = calibrated min-µs end, 10000 = max-µs end) and the role maps it LINEARLY onto its LIVE calibrated `[minUs,maxUs]` (`ServoActuatorRole::setNormalizedTarget`), reflecting for REV. The effect never holds the limits, so a re-calibration is honoured on the next command with zero plumbing — GunFx yaw/pitch convert their RC pulse → fraction at the edge (the only spot that knows it's RC), LandingFx deploy/retract send full/zero. This is the canonical "intent in, mechanism owned by role" shape; prefer it over sending an absolute `setTarget(us)` from an effect that would have to know the calibration (the absolute path is how the "deploy stops short of the calibrated endpoint" bug arose — fixed by routing through the normalised verb + the 2.21.2 `rebuildProfileLimits` assign-not-clamp fix). Two firmware invariants the role must keep: (a) `rebuildProfileLimits` ASSIGNS `_profile.min/max = _minUs/_maxUs` in both directions (the integrator clamps to the profile window, which must equal the calibration window); (b) `CONFIG_RELOAD` does NOT re-attach an already-bound role, so only the LIVE `SERVO_SET_PROFILE` push updates a running servo's limits — a persisted-only change waits for the next boot/attach. See CLAUDE.md "Specific gotchas".
