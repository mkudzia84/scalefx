...
Multi-platform embedded effects: one **ESP32-S3 HubFX** master + up to N **Pico (RP2040)** slaves (GunFX, LightFX, GearControl) over USB CDC at 6 Mbps.
...
17. **Policy-based templates for compile-time dispatch (Rule 18).** `StorageServerT<TPolicy>`, `SdCardModuleT<TPolicy>`. `SharedState` struct + `policy()` accessor + `using FooServer = FooServerT<PlatformFooPolicy>`. No virtual when only one instance exists per binary.
...
- **INIT_READY / IDENTIFY (0xFE)** — same length-prefixed payload (`nameLen:u8, name, verLen:u8, ver, platLen:u8, plat, cpuMHz:u32LE, freeRam:u32LE, buildNum:u32LE`). IDENTIFY does NOT activate hardware → used for safe type detection; INIT activates. HubFX auto-inits at boot (IDENTIFY suffices); slaves still need INIT.
...
- **Upload protocol — use WINDOWED (mode=2), not STREAM (mode=3).** STREAM counts raw bytes at 6 Mbps and desyncs irrecoverably above ~1 MB. WINDOWED uses per-window ACK + flow control + per-chunk CRC-16, default window size 32 (~64 KB). See [instructions/10-UPLOAD-PROTOCOL-REFACTOR.md](instructions/10-UPLOAD-PROTOCOL-REFACTOR.md).
