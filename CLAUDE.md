...
Authoritative rulebook: [.github/copilot-instructions.md](.github/copilot-instructions.md) (Rules 0–62; all GunFX Phase 4 rules landed; Rules 53–56 = wire-async / upload-exclusivity / native-hardware / thread-safe-wire; Rule 57 = upload self-heal; Rule 58 = transparent expander roles (opaque PortRef-addressed transport — drive/query/telemetry of any role on any board behaves hub-local); Rule 59 = every Studio tab routes to a DEDICATED panel (no generic fallback — un-hooked domains render NotImplementedTab; verify the id chain domainCatalog → studioTabs → MainLayout by clicking the tab); Rule 60 = panel layout grammar (two-column unit cards: ports LEFT / behaviour RIGHT; live widgets full-width; ≤4-option mode choices = `.seg-select` segmented toggles, radios retired; sibling-aware unclaimed pools; ops in header; param packs in grids); Rule 61 = Studio sub-tabs use the shared `.sub-bar`/`.sub-item` underline pattern (not a `.seg-select` pill — sub-tabs switch the view; CSS global, mount only the active surface, conditional sub-tabs OK); Rule 62 = every Studio tab's content area uses a standard 12px margin (no 20/24px one-offs, no double-padding). **HubFX is pure ESP-IDF now — `framework = espidf`, zero Arduino.**).
...
- **HubFX master dispatch map (the authoritative, conflict-validated allocation — every packet a `BoardOf<>` policy `ownsType()` on the hub; dispatch STOPS at the first policy whose `ownsType` returns true, so NO two policies may claim the same byte).** Validate against this before adding a packet; append at the next free value, never into a neighbour's block:
  ...
  - **Audio control `0xDA–0xE1`** · **Audio preload diag `0xE6–0xE7`** · **Input routing `0xE8–0xEA`** (global RC→effect gate, InputDispatcher) · **Telemetry collection `0xEB–0xED`** (read the JetiTelemetryHub — InputDispatcher, 2026-06-13) · BatteryConfig `0xEE` · Board/Core lifecycle `0xEF–0xFF`
  - **Input connection-loss `0xAD–0xAF` + `0xA8`** (CONNECTION_EVENT/GET/RESP + SET_CFG — generic link-loss detection, InputDispatcher, 2026-06-13)
  - Free: `0x00–0x0F`, `0xD8–0xD9` (ex-GearCalibrate).
  - ⚠️ `0xA8`/`0xAD–0xAF` are PACKET-TYPE bytes here (connection-loss); they're a SEPARATE namespace from the same-valued StorageError ERROR CODES (`0xA0–0xAF`) — wire-OK, but don't confuse the two when grepping.
...
Authoritative rules with full rationale + examples: [.github/copilot-instructions.md](.github/copilot-instructions.md) (Rules 0–62). Below is the compact index — **same rule numbers**, one line each. When this conflicts with copilot-instructions.md, that file wins. Architecture overview: [32-ARCHITECTURE-DIAGRAMS.md](instructions/32-ARCHITECTURE-DIAGRAMS.md).
...
- **60** Panel layout grammar: per-unit cards = two-column grid split by SUBSYSTEM (one subsystem per column — its ports + calibration + behaviour together; gun core|smoke, gear motor|doors; collapse <900px); live-telemetry widgets ALWAYS full-width (grow wrapper — bare widget in a form-row takes content width); mode choices ≤4 options = `.seg-select` segmented toggle (radios retired); pools feed `$effectClaims` NOT the vestigial `$deviceModel.claims` (new effect adds its block to effect-claims.ts); ops in the card header; ≥2 related numerics = `.form-grid`; backgrounds = exactly 3 levels (panel `.card` → unit `.card` → `.sub-frame` --bg-raised + frame-head; never bg-raised on a unit card). Reference: GearPanel strut cards.
- **61** Studio sub-tabs (sibling surfaces of one top-level tab — Lighting Programs|Retractable, Input&Ports Config|Telemetry) use the shared global `.sub-bar`/`.sub-item` underline pattern (style.css), NOT a `.seg-select` pill (that's for SETTING choices; sub-tabs SWITCH THE VIEW): icon+label items, accent bottom-border on `.active`, mount only the active surface, CSS lives once globally (Rule 34), conditional sub-tabs OK (default first), per-surface DirtySource (Rule 46). Reference: LightingTab / IoTab.
- **62** Studio tabs use a standard **12px** content margin — every top-level tab's scroll area pads 12px all sides (`.tab-content` OR a self-managed scroll); no other outer padding (20px/24px one-offs retired), no double-padding (a `.tab-content`-wrapped component's root sets only `max-width`, not padding).
...
- **`CONFIG_RELOAD` now DOES rebuild the hub-local port/role layout (v2.29.0, 2026-06-13).** The `/hubfx.yaml` store apply callback (`applyHubFxConfigCallback`) runs FIRST in the reload loop and — post-boot only (gated on `g_lightFxBooted`) — calls `TopologyService::detachAllHubRoles()` then `applyPortRoles(...)`, so a Studio *Apply* (save `/hubfx.yaml` + reload) re-attaches every hub-local role from the fresh config (changed roles, added/removed ports, AND `ports[].profile` via the fresh `emplace`) WITHOUT a reboot. The detach releases claims; the effect stores' reload callbacks then re-claim. The reload-complete hook runs `reassertHubFxFeatures` (features only — NOT the full callback) so it doesn't re-detach what the effects just re-claimed. Bench-verified: reload cycles the hub roster 20⇄8 live (CLI `topo-roles`). The **live** `SERVO_SET_PROFILE` push (Studio calibration Save / `SetPortProfile`) is still the lightest path for a single servo-limit tweak (no full detach/reattach). Diagnose with the `[config] reload: detached N hub role(s)` / `[servo] setprofile …` instrumentation.
- **Hub-local `detachRole`/`attachRole` dispatch LOCALLY** (`sendRoleCommand` → `isLocalTarget(empty guid)` → `RoleServicePolicy::handle` in capture mode), so `detachAllHubRoles()` walking the port registry (`numServoPorts`/`servoAt`, …, each occupied `PortRef::local(kind, i)`) tears down the hub's OWN roles without touching expander boards — expander rosters (e.g. GearControl GUID `3225`) are untouched across a reload.
- **A missing / empty (0-byte) config on reload now applies defaults + fires the apply callback (v2.29.0).** `ConfigStore::loadFromFile` (read ≤0 bytes) and `loadFromString` (empty input) both `resetToDefaults()` + fire `_onLoaded` + flag `result.notFound`; single-path `CONFIG_RELOAD` treats `notFound` as ACK (not NACK). So a reload of a board whose `/hubfx.yaml` is absent/0-byte gracefully reverts to the schema-default layout (LED-animator on every PWM) instead of leaving stale roles or wedging. `ConfigStoreSlot::loadOrFallback` skips its own second apply when the store already self-fired (`notFound`).
- **Initial-flash config seeding (`scalefx-flash`, v2.29.0).** `cmdFlash hubfx` (and the manual `scalefx-flash seed-config`) uploads the bundled `media/presets/hubfx/minimal.yaml` → `/hubfx.yaml` ONLY when the board has none. **It probes existence on the FLASH backend via `Storage.FileInfoOn(path, TargetFlash)`** — the plain `FileInfo` defaults to SD (where config files don't live) and would read every config as "absent" → clobber the operator's real config (this bit during bench: it overwrote a live `/hubfx.yaml` twice). The seed fails SAFE: a `FileInfo` error (transient on a just-rebooted board) ⇒ skip seeding, never overwrite. LittleFS survives a reflash, so a reflash of a configured board is preserved untouched.
