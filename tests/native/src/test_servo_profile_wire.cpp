...
// codec down here is the regression net for the REV-on-attach class of bug
// (pack set `inverted`, an unpack site ignored it).
...
TEST_CASE("ServoProfileWire pack→unpack round-trips every field") {
    ...
    in.inverted          = true;        // the field the old attach path dropped
    ...
    CHECK(out.inverted          == in.inverted);   // must survive the round-trip
...
TEST_CASE("ServoProfileWire unpack is Rule-11 append-only (short payloads keep defaults)") {
    ...
    in.inverted = true; in.centerUs = 1480;
    in.maxAccelUsPerSec2 = 222; in.maxJerkUsPerSec3 = 99;
    uint8_t buf[ServoProfileWire::kSize];
    ServoProfileWire::pack(buf, in);

    // A 7-byte payload carries min/max/speed/inverted but NOT centre/accel/jerk.
