/**
 * SFX Test (P-variant) — TAS5825P Audio Codec Test Firmware
 *
 * Test firmware for verifying TAS5825P codec operation on the HubFX
 * board. Generates a 440 Hz sine wave via I2S, initializes the
 * TAS5825P via I2C, and prints extensive codec diagnostics to Serial.
 *
 * Despite the historical `TAS5825M_*` naming on the symbols below,
 * the init flow this file performs has only been validated against
 * the TAS5825P silicon (Class-H + Hybrid-Pro variant, package suffix
 * RHB) — that's the chip on the original HubFX bring-up board. The
 * P shares register addresses with the M but does not enforce the
 * M-specific guards (smart-amp speaker-protection latch, FS_MON
 * validity gate before PLAY) that this same register stream
 * stumbles over on M silicon.
 *
 * The symbol names are kept TAS5825M_* so the original code reads
 * intact and a future renaming pass can be a single search-and-
 * replace. The M-specific firmware lives at ../sfx_test_m/.
 *
 * NO external libraries — only Arduino core + ESP-IDF headers.
 * Works in both Arduino IDE (board: "ESP32S3 Dev Module") and PlatformIO.
 *
 * Hardware: ESP32-S3R8 (HubFX board, TAS5825P silicon)
 *
 * I2S Pin Mapping (wired to TAS5825P codec):
 ...
 *   GPIO9  — I2C SCL
 *
 * TAS5825P Codec: I2C address 0x4C
...
static void printBanner() {
    Serial.println();
    Serial.println("================================================================");
    Serial.println("  SFX Test (P-variant) — TAS5825P Sine Wave I2S Test");
    Serial.println("  ESP32-S3 / HubFX board  |  Class-H + Hybrid-Pro silicon");
    Serial.println("  Init flow: permissive (no smart-amp / FS_MON gating)");
